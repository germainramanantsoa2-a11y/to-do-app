...
DEFAULT_USERS = {
  "Germain": {"password": hashlib.sha256("1234".encode()).hexdigest(), "role": "Germain", "name": "Germain"},
...
def change_password(users):
    ...
    confirm_pwr = st.text_input("confirmer le mot de passe", type="password", key="confirm")

    if st.button("Valider"):
        username = st.session_state.username
        if users[username]["password"] != hash_password(old_pwd):
            st.error("Ancie mot depassez incorrect")  
        elif new_pwd!= confirm_pwr:
...
for i, t in enumerate(st.session_state.tasks):
    ...
    with st.container(border=True):
        ...
        with col2:
            if st.session_state.role == "Germain":
